...
I learned that there wasn't a "add_executable" command anymore in the CMakeLists.txt, so after some time trying to figure it out and **asking questions** about it in the group and to Daan, i figured out i needed to build from the top-level where JUCE was in and then navigate to the example_artifacts to run ```make``` and then run the executable.  
...
# Sineing the Inheritance

Since there is now a lot of duplicate code (same members in different classes defined), the right option was to add an Oscillator class (abstract). In **simpleClassDiagram.png**, you can see the inheritance (with member functions) I wanted to work towards for this assignment. I started by linking the **Sine class to the Oscillator class**.  
...
*Essentially, oscillator is an abstract class object that initialises anything to do with the oscillators itself. The derived classes use all this information, but also override the calculate() function to activate which waveform is going to be used in the sound.*  
...
Aaaand I had to deal with lots of different errors and fixing one after the other but i got there in the end.  

# Squaring up the Inheritance

Now i'm adding the square in the inheritance too. I'm doing this without looking at the things i did with the sine, because i want to see if i can get there without any help.  

Results were pretty good! Ofcourse i forgot some things:
- Include oscillator.h
- No need to define any private field in header like frequency and samplerate

I'm proud to say these were the only 2!  

# Sawing the Inheritance
